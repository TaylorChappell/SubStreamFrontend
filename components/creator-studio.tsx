...
const previewMarkets: CreatorMarket[] = [
  { id: "demo-tidal", name: "Tidal", symbol: "TIDAL", mint: "AQVcP67EpMyu4cBZZjqMu91cVsWy1aX98JmcZm1FyY9", marketCapUsd: 284000, priceUsd: .00483, aquaUrl: "https://aqua-launchpad.xyz/market/demo-tidal", streamId: "12c75bb4-2041-484a-9bab-a8c89ebc8901", streamStatus: "live", streamTitle: "Building the next rewards dashboard", slug: "tidal", viewerCount: 184 },
  { id: "demo-wave", name: "Wave Labs", symbol: "WAVE", mint: "WaVeGxJjJ2wYvFg6zi22UU5Qi38fGMxV3ewwPDXxAAA", marketCapUsd: 92000, priceUsd: .00156, aquaUrl: "https://aqua-launchpad.xyz/market/demo-wave" },
];

const previewChannels: CreatorChannel[] = [
  { id: "12c75bb4-2041-484a-9bab-a8c89ebc8901", marketId: "demo-tidal", marketName: "Tidal", symbol: "TIDAL", slug: "tidal", title: "Building the next rewards dashboard", description: "Product walkthrough, holder questions and a first look at what ships this week.", category: "Development", status: "live", viewerCount: 184 },
];
const earliestScheduleAt = new Date(Date.now() + 10 * 60_000).toISOString().slice(0, 16);
...
export function CreatorStudio() {
  ...
  const [markets, setMarkets] = useState<CreatorMarket[]>(previewMarkets);
  const [channels, setChannels] = useState<CreatorChannel[]>(previewChannels);
  const [selectedId, setSelectedId] = useState(previewChannels[0]!.id);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [credentials, setCredentials] = useState<Credentials | null>(null);
  const [credentialsVisible, setCredentialsVisible] = useState(false);
  const [title, setTitle] = useState(previewChannels[0]!.title);
  const [description, setDescription] = useState(previewChannels[0]!.description);
  const [category, setCategory] = useState(previewChannels[0]!.category);
  const [announcement, setAnnouncement] = useState("AMA starts in 10 minutes. Drop your holder questions in chat.");
  const [scheduleAt, setScheduleAt] = useState("");

  const load = useCallback(async () => {
    if (!connected || !API_CONFIGURED || !token) return;
    setLoading(true);
    try {
      ...
      setMarkets(nextMarkets); setChannels(nextChannels);
      if (nextChannels[0]) setSelectedId(nextChannels[0].id);
  ...
  const createChannel = async (market: CreatorMarket) => {
    ...
    try {
      if (API_CONFIGURED) {
        ...
        const channel: CreatorChannel = { id: `preview-${market.id}`, marketId: market.id, marketName: market.name, symbol: market.symbol, slug: market.symbol.toLowerCase(), title: `${market.symbol} community live`, description: `Live updates and holder questions from ${market.name}.`, category: "Community", status: "offline", viewerCount: 0 };
        setChannels((items) => [...items, channel]); setMarkets((items) => items.map((item) => item.id === market.id ? { ...item, streamId: channel.id, streamStatus: channel.status, streamTitle: channel.title, slug: channel.slug } : item)); setSelectedId(channel.id);
      }
  ...
  const saveChannel = async (event: FormEvent) => {
    ...
    try {
      if (API_CONFIGURED) await apiFetch(`/api/creator/streams/${selected.id}`, { method: "PATCH", body: JSON.stringify({ title, description, category }) }, token);
  ...
  const revealCredentials = async () => {
    ...
    try {
      const next = API_CONFIGURED
        ? (await apiFetch<{ credentials: Credentials }>(`/api/creator/streams/${selected.id}/credentials`, {}, token)).credentials
        : { server: "rtmps://live.cloudflare.com:443/live", streamKey: `sub_live_preview_${selected.symbol.toLowerCase()}_4f8d2a` };
  ...
  const rotateKey = async () => {
    ...
    try {
      const next = API_CONFIGURED
        ? (await apiFetch<{ credentials: Credentials }>(`/api/creator/streams/${selected.id}/rotate-key`, { method: "POST" }, token)).credentials
        : { server: "rtmps://live.cloudflare.com:443/live", streamKey: `sub_live_preview_${crypto.randomUUID().slice(0, 12)}` };
  ...
  const publishAnnouncement = async () => {
    ...
    try {
      if (API_CONFIGURED) await apiFetch(`/api/creator/streams/${selected.id}/announcement`, { method: "PUT", body: JSON.stringify({ body: announcement.trim() }) }, token);
  ...
  const scheduleStream = async () => {
    ...
    try {
      if (API_CONFIGURED) await apiFetch("/api/creator/schedules", { method: "POST", body: JSON.stringify({ marketId: selected.marketId, streamId: selected.id, title: selected.title, description: selected.description, scheduledFor, durationMinutes: 60 }) }, token);
  ...
  const endStream = async () => {
    ...
    try {
      if (API_CONFIGURED) await apiFetch(`/api/creator/streams/${selected.id}/end`, { method: "POST" }, token);
    ...
    <section className="studio-workspace">
      ...
      {selected ? <>
        ...
        <div className="studio-grid">
          ...
          <div className="studio-card"><div className="studio-card-heading"><span><CalendarPlus size={18} /></span><div><h2>Schedule the next stream</h2><p>Appear on the public calendar before you go live.</p></div></div><label>Date and time<input type="datetime-local" value={scheduleAt} min={earliestScheduleAt} onChange={(event) => setScheduleAt(event.target.value)} /></label><div className="schedule-preview"><CalendarPlus size={20} /><span><b>{scheduleAt ? new Date(scheduleAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" }) : "Choose a time"}</b><small>Default duration: 60 minutes</small></span></div><button className="primary-action secondary" type="button" disabled={!connected || !scheduleAt} onClick={() => void scheduleStream()}>Add to schedule</button></div>
        </div>
      </> : <div className="studio-empty"><Video size={34} /><h1>Create your first channel</h1><p>Your wallet is verified, but none of its AQUA coins have a Sub Stream channel yet.</p></div>}
      <div className="studio-footnote"><span>Signed in as</span><code>{wallet ? shortMint(wallet) : "wallet not connected"}</code><span>·</span><span>{API_CONFIGURED ? "Production API" : "Interactive preview data"}</span></div>
    </section>
  </div>;
}
