...
export function AppHeader() {
  ...
  return <>
    ...
    <CommandDialog open={searchOpen} onOpenChange={setSearchOpen} title="Search Sub Stream" description="Find a live AQUA community">
      <CommandInput placeholder="Search coins or streams…" />
      <CommandList><CommandEmpty>No live streams found.</CommandEmpty><CommandGroup heading="Live now">
        {demoStreams.map((stream) => <CommandItem key={stream.id} value={`${stream.market.symbol} ${stream.market.name} ${stream.title}`} onSelect={() => navigate(`/stream/${stream.slug}`)}><Radio className="text-[#ff7168]" /><span><b>{stream.market.symbol}</b> · {stream.title}</span><small className="ml-auto text-muted-foreground">{stream.viewerCount}</small></CommandItem>)}
