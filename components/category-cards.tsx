...
export function CategoryCards() {
  ...
  return <div className="category-grid">{CATEGORIES.map(item=>{const stat=stats.find(s=>s.name===item.name);return <Link className={`category-card category-${item.id}`} href={`/category/${item.id}`} key={item.id}><div className="category-poster" aria-hidden="true"><strong>{item.word}</strong><div className="poster-art"><i/><i/><i/><i/></div><span className="poster-caption">{item.short}</span></div><h3>{item.label}</h3><p>{stat?stat.live?`${stat.live} live ${stat.live===1?'channel':'channels'}`:`${stat.channels} ${stat.channels===1?'channel':'channels'}`:item.short}</p></Link>;})}</div>;
}
