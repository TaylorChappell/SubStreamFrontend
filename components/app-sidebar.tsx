...
export function AppSidebar() {
  ...
  return <aside className="app-sidebar"><nav aria-label="Primary navigation">{nav.map(([href,label])=><Link key={href} href={href} aria-current={path===href?'page':undefined} className={path===href||(href==='/categories'&&path.startsWith('/category/'))?'active':''}>{label}{href==='/'&&<span className="nav-live-dot"/>}</Link>)}</nav><div className="sidebar-section"><h2>{token?'Your channels':'Around the stream'}</h2>{channels.map(c=><Link className="sidebar-channel" key={c.id} href={`/stream/${c.slug}`}><CoinAvatar symbol={c.market.symbol} src={c.market.imageUrl} size="small"/><span><b>{c.market.symbol}</b><small>{c.status==='live'?c.title:'Offline'}</small></span>{c.status==='live'&&<i className="nav-live-dot"/>}</Link>)}{!channels.length&&<p>{failed?'Channels are temporarily unavailable.':token?'Follow a channel to keep it close.':'A place for every AQUA community.'}</p>}</div><div className="sidebar-bottom"><Link className={path==='/studio'?'active':''} href="/studio">Creator studio <span>↗</span></Link><div className="sidebar-aqua"><span>AQUA ECOSYSTEM</span><p>Good communities<br/>keep flowing.</p></div></div></aside>;
}
