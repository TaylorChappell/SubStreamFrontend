...
export function FollowButton({ marketId, initial = false }: { marketId: string; initial?: boolean }) {
  ...
  return <button className={`follow-button ${followed?'following':''}`} disabled={busy || Boolean(token && !ready)} aria-pressed={followed} onClick={() => void toggle()}>{followed?'Following':'+ Follow'}</button>;
}
